...
//!
//! A Jupyter kernel for executing vvSQL queries with rich Vega-Lite visualizations.
...
#[derive(Parser)]
#[command(name = "vvsql-jupyter")]
#[command(about = "Jupyter kernel for vvSQL", long_about = None)]
struct Args {
...
async fn main() -> Result<()> {
    ...
    tracing::info!("vvSQL Jupyter Kernel v{}", env!("CARGO_PKG_VERSION"));
...
fn install_kernel(user: bool, sys_prefix: bool) -> Result<()> {
    println!("Installing vvSQL Jupyter kernel...");

    // Create a temporary directory for the kernel spec
    let temp_dir = env::temp_dir().join("vvsql-kernel-install");
    if temp_dir.exists() {
    ...
    println!(
        "Running: jupyter kernelspec install {} --name vvsql{}",
        temp_dir.display(),
        if user || (!sys_prefix) {
            " --user"
        } else {
    ...
    if status.success() {
        println!("\n✓ vvSQL kernel installed successfully!");
