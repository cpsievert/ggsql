...
This test suite validates that vvsql-jupyter implements the Jupyter
...
class VVSQLKernelTests(jkt.KernelTests):
    """Compliance tests for vvsql-jupyter kernel."""
    ...
    def setUp(self):
        ...
        result = subprocess.run(
            ["cargo", "build", "--bin", "vvsql-jupyter"],
            cwd=repo_root / "vvsql-jupyter",
            capture_output=True,
            text=True,
        )
    ...
    def test_kernel_info(self):
        ...
        self.assertEqual(content["implementation"], "vvsql")

        # Language info
        lang_info = content["language_info"]
        self.assertEqual(lang_info["name"], "vvsql")
        self.assertEqual(lang_info["file_extension"], ".vvsql")
        self.assertEqual(lang_info["mimetype"], "text/x-vvsql")
...
def setup_module():
    ...
    # Find binary
    binary_path = repo_root / "target" / "debug" / "vvsql-jupyter"
    ...
    spec_dir = Path(tempfile.mkdtemp(prefix="vvsql-kernel-"))
...
def teardown_module():
    ...
    subprocess.run(
        ["jupyter", "kernelspec", "remove", "-f", "vvsql"],
        capture_output=True,
    )
