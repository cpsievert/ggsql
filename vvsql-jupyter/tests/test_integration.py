"""
Integration tests for vvsql-jupyter kernel using jupyter_client.
...
def kernel_binary():
    """Build and return path to vvsql-jupyter binary."""
    # Build the kernel
...
class TestKernelInfo:
    ...
    def test_kernel_info_request(self, client):
        ...
        assert content["implementation"] == "vvsql-jupyter"

        # Check language info
        lang_info = content["language_info"]
        assert lang_info["name"] == "vvsql"
        assert lang_info["file_extension"] == ".vvsql"
        assert lang_info["mimetype"] == "text/x-vvsql"
